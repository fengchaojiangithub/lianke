...
<html>
  ...
  <div class="all">
    <div class="content">
      <div class="content_new">
       ...
       <img src="/Public/Index/img/true_03.jpg" id="true" style="margin:20px 0 10px 280px;"/>
        
        <div class="my">
          悬赏分：<span class="money1"><input type="text" id="money" name="money" /></span> <span class="user_money">你有<a><?php echo ($data["user_money"]); ?>金币</a>,<span class="whole">全部悬赏</span>全部悬赏</span>  
        </div>
      </div>
    </div>
  </div>
  ...
  $('#kg').on('click','li',function (event){
      ...
      // alert($(this).attr('id'));
      if(bt.length == 1){
        $('.inpt3').append('<button type="button" id="lab_1" value='+$(this).attr('id')+'>'+$(this).text()+'</button>');
      $('.ht').css('width','390px');
      $('.ht').attr('placeholder','点击添加第二个话题');
      }else if(bt.length == 2){
        $('.inpt3').append('<button type="button" id="lab_2" value='+$(this).attr('id')+'>'+$(this).text()+'</button>');
        $('.ht').css('width','290px');
        $('.ht').attr('placeholder','点击添加第三个话题');
      }else if(bt.length == 3){
        $('.inpt3').append('<button type="button" id="lab_3" value='+$(this).attr('id')+'>'+$(this).text()+'</button>');
        $('.ht').css('width','190px');
        $('.ht').attr('placeholder','');
      }     
  });
  ...
  $('#true').click(function(){
      ...
      $.ajax({
                  type:"post",
                  url:"<?php echo U('Question/question_add');?>",
                  data:{title:title,lab_id:lab_id,content:content,money:money},
                  success: function(msg){
                    // alert(msg)
                          if (msg==0) {
                            alert('您的金币不足');
